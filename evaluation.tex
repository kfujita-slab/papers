...
溢れた分は他の資源を用いて合成される。DSPは主に固定小数点数の乗算に
...
\begin{table}[hbt]
    ...
    \begin{tabular}{crrr}
        \hline
        資源    &\multicolumn{1}{c}{使用量}  & \multicolumn{1}{c}{使用可能} &\multicolumn{1}{c}{割合}     \\\hline\hline
...
Report Timing Summaryを利用して評価する。
論理合成段階での判定であるため、
...
注目画素の入力から概ね画像横幅分のレイテンシが必要となる。
さらに、本システムではpooling・unpoolingを行うため、これらの操作によって
も必要なレイテンシは変化する。
\begin{figure}[hbt]
    \centering
    \includegraphics[width=0.85\textwidth,clip]{image/juyoya_image.pdf}
...
これを踏まえ、Verilogシミュレーションであるxmverilogで
全体のレイテンシを検証する。
...
入力の座標値に対し$154\mathrm{行}+57$クロック分のレイテンシを経て出力されている
ことが読み取れる。
...
\begin{table}[hbt]
    ...
    \begin{tabular}{crrrr}
        \hline
        資源    &\multicolumn{1}{c}{ExtNet}  & \multicolumn{1}{c}{RdcNet} &\multicolumn{1}{c}{ItgNet} &\multicolumn{1}{c}{1$\times$1conv}     \\\hline\hline
...
小規模ネットワークの再帰的利用を特徴とする。
poolingによってLEVELが高くなるにつれ、
有効画素が入力されるタイミングは減少し、
...
全体の資源使用量の多くを占める本実装において、効果的な削減手法といえる。
...
separable convolution\cite{separable}は、
畳み込み演算を空間方向とチャネル方向に分離
させることで、パラメータ数と計算量を削減させる手法である。
図\ref{fig:separable}に、画像の大きさを$F$、フィルタの大きさを$K$、
チャネルの大きさを$M$と$N$として出力例を示す。
...
\begin{gather}
    ...
    K^{2}M + MN\quad(\mathrm{separable\ convolution})
\end{gather}
%KはNに比べて小さいため、計算量は約$1/\mathrm{K}^2$倍となる。
計算量は$\frac{K^{2}+N}{K^{2}N}$倍となり、一般的に
$K$は$N$に比べ小さいため計算量の大きな削減が見込める。
