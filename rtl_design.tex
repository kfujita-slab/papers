...
それ以外のモジュールでは入出力に対して図\ref{fig:LEVEL_design}に示したように
タイミングをとる。
\begin{figure}[hbt]
    \centering
...
また、stream\_patchが出力すべき座標値は、そのクロックで入力された座標値とstream\_patchモジュールのレイテンシを利用した計算によって求められる。
...
\begin{align}
\mathrm{patch\_streamモジュールのレイテンシ} = \mathrm{LATENCY}\times 2^{\mathrm{LEVEL}}
\label{LEVEL_LATENCY}
\end{align}
...
そこで、図\ref{fig:NN_image}に表したような、層間におけるニューロンの計算を
...
\begin{align}
    \mathrm{畳み込み演算のレイテンシ} = \log{2}(\mathrm{PATCH\_NUM}\times\mathrm{PREV\_NEURONS})
    \label{layer_LATENCY}
\end{align}
\begin{figure}[hbt]
    \centering
...
自身のLEVELと出力する座標値を用いた判定によって決定することができる。
...
いわばLEVELを下げる処理を行っているといえる。
そのため、出力するイネーブル信号の判定はLEVELが変化するという前提で行う必要が
あり、同LEVELのlayerモジュールの判定とは違ったものとなる。
...
まず、座標値についてだが、左上方向への拡張は有効画素が入力されたときに
出力することが可能である。
したがって、入力される有効画素の座標値に対し、式\ref{unpool_outcnt}
で表されるだけのレイテイシが存在すると仮定した。このレイテンシを基に
...
out\_hcnt,\ out\_vcntは各ItgNetモジュール前段のunpoolingモジュールの出力が渡される。
