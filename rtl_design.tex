...
有効画素以外をメモリに格納する必要がないため、イネーブル制御を用いず、
...
よって、ネットワーク全体の入出力、layerモジュールの段数、各layerモジュールの
...
%LEVELを用いた式によって示す。
入力された画素を拡張された左上の画素とみなすため、残りの3画素については
%適切な大きさのバッファを通して出力すればよい。
バッファメモリを使った遅延によって出力する。
適切なバッファの大きさはLEVELを用いた式で以下のように表せる。

\begin{itemize}
\item 右上方向 : UppR\_BUF = $2^{\mathrm{LEVEL}}$
\item 左下方向 : LowL\_BUF = $2^{\mathrm{LEVEL}} \times$WIDTH
